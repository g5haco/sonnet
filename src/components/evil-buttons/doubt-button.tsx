...
import { useAshBurst } from "@/components/evil-buttons/ash-burst-button";
...
export const DoubtButton = React.forwardRef<HTMLButtonElement, DoubtButtonProps>(
  (
    ...
    // Sonnet: the final confirmation erupts in ash (merged from ash-burst-button).
    const ash = useAshBurst();
    ...
    const fire = () => {
      ...
      setState("success");
      setIndex(0);
      if (innerRef.current) ash.burst(innerRef.current);
      onConfirm?.();
      if (resetAfter > 0) {
    ...
    const accentAlpha = 0.12 + t * 0.3; // Sonnet: capped so the red label stays readable on the red fill
    ...
    return (
      <motion.span className="relative inline-flex" animate={ash.controls}>
        {ash.overlay}
        <button
          ref={setRef}
          type={type}
          ...
          className={cn(
            ...
            isSuccess
              ? "border-done/40 bg-done/10 text-done focus-visible:ring-done"
              : "cursor-pointer text-destructive focus-visible:ring-destructive",
            className,
          )}
          style={
            isSuccess
              ? undefined
              : {
                  borderColor: `color-mix(in oklch, var(--destructive) ${Math.round(ringAlpha * 100)}%, transparent)`,
                  backgroundColor: `color-mix(in oklch, var(--destructive) ${Math.round(accentAlpha * 100)}%, transparent)`,
                }
          }
          {...props}
        >
          <motion.span
            key={shakeKey}
            animate={reduceMotion || shakeKey === 0 || isSuccess ? undefined : { x: [0, -6, 6, -4, 4, -2, 2, 0] }}
            ...
            <span className="relative inline-grid">
              ...
              <span aria-hidden className="invisible col-start-1 row-start-1 whitespace-nowrap">
                {[label, successLabel, ...confirmations].reduce<React.ReactNode>(
                  (widest, candidate) =>
                    String(extractText(candidate)).length > String(extractText(widest)).length ? candidate : widest,
                  "",
                )}
              </span>
            ...
            <span className="flex items-center gap-1" aria-hidden>
              {Array.from({ length: steps }).map((_, i) => (
                <span
                  ...
                  style={{
                    backgroundColor:
                      i < index ? "var(--destructive)" : "color-mix(in oklch, var(--destructive) 25%, transparent)",
                  }}
                />
              ))}
            </span>
