...
function drawShard(ctx: CanvasRenderingContext2D, size: number, color: string) {
  ctx.fillStyle = color;
  ctx.beginPath();
  ctx.moveTo(0, -size);
...
function createAshSimulation(
  ...
  const lipW = Math.max(10, w * 0.1);
  const lipH = 7;
  const leftLip = Bodies.rectangle(rect.left + lipW / 2, rect.top - lipH / 2 + 1, lipW, lipH, {
    isStatic: true,
    friction: 1.4,
    restitution: 0.04,
    label: "ash-lip",
  });
  const rightLip = Bodies.rectangle(rect.right - lipW / 2, rect.top - lipH / 2 + 1, lipW, lipH, {
    isStatic: true,
    friction: 1.4,
    restitution: 0.04,
    label: "ash-lip",
  ...
  for (let i = 0; i < count; i++) {
    ...
    const kind: ParticleKind =
      kindRoll > 0.93 ? "skull" : kindRoll > 0.72 ? "shard" : kindRoll > 0.4 ? "square" : "circle";
    ...
    const speed = options.startVelocity * (0.45 + Math.random() * 0.55) * 0.7;

    // Mostly explode from center; a few rain down later to settle on top.
    const rain = Math.random() > 0.72;
    const spawnX = rain ? rect.left + Math.random() * w : cx + (Math.random() - 0.5) * w * 0.4;
    const spawnY = rain ? rect.top - 8 - Math.random() * 28 : cy + (Math.random() - 0.5) * h * 0.3;
...
function paintSim(sim: AshSim, button: HTMLElement) {
  ...
  for (const body of Composite.allBodies(engine.world)) {
    ...
    if (fade <= 0 || body.position.y > h + 48 || body.position.x < -48 || body.position.x > w + 48) {
      toRemove.push(body);
      continue;
    }
...
// Merged into DoubtButton (Sonnet): the ash burst is a hook, so the doubt button can fire it on its
// final confirmation. `overlay` must be rendered; `burst(el)` erupts from that element.
export function useAshBurst({
  particleCount = 96,
  spread = 120,
  startVelocity = 48,
  colors = ASH_COLORS,
  ...
  const burst = React.useCallback(
    (button: HTMLElement) => {
      ...
      stopSim();
      setSimActive(true);
      const sim = createAshSimulation(canvas, button, { particleCount, spread, startVelocity, colors });
      simRef.current = sim;
      const tick = () => {
        const current = simRef.current;
        if (!current) return;
        if (paintSim(current, button)) current.raf = requestAnimationFrame(tick);
        else stopSim();
      };
      sim.raf = requestAnimationFrame(tick);
      // the button scorches, then springs back
      void controls
        .start({
          scale: 0.78,
          opacity: 0.35,
          filter: "brightness(0.25) contrast(1.6) saturate(2.4) hue-rotate(-12deg)",
        ...
        .then(() =>
          controls.start({
            ...
            transition: { type: "spring", stiffness: 360, damping: 20, mass: 0.55 },
          }),
        );
    },
    [colors, controls, particleCount, reduceMotion, spread, startVelocity, stopSim],
  );
