...
export async function proxy(request: NextRequest) {
  ...
  if (!data?.claims && !["/login", "/auth", "/api/cal", "/api/cron", "/robots.txt", "/sitemap.xml", "/opengraph-image", "/icon", "/apple-icon", "/manifest.webmanifest"].some((p) => path === p || path.startsWith(`${p}/`))) {
