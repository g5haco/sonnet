...
| 5   | **Canvas sync**                 | Courses, assignments (with descriptions, so the AI can summarize them) and grades pulled from Canvas; Sync now; daily auto-sync                                                                     | Apply migration 0005; add the three server secrets; connect with the token + calendar URL | 🟡 Built: encrypted token + ICS sync, status UI, daily cron and AI descriptions; needs live setup and first real sync                 |
...
**Next: finish Phase 5 live setup and verify the first real Canvas sync; then continue Phase 6 AI material reading and syllabus import.**
